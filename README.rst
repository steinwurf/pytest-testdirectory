...
For example with our native cod (C/C++) we compile as much as possible from
...
Configures, build and run any available tests for a given project, such that you
as a developer can start hacking at the code.

For Python project this is a

We've used  to "automate" some aspects of the development.

To get started use::

    ./waf configure

This will download e.g. ``virtualenv`` to the local machine, such that it can
be used when testing the package.
...
This follows what seems to be "best practice" advise is to install the package
in editable mode in a virtualenv.
